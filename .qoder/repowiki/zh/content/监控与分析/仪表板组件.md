...
## 更新摘要
**变更内容**
- 新增日期范围过滤支持：所有统计接口（getStats、getRecentActivity、getUsageTrend、getRegionDistribution、getRecentIpRequests、getModelDistribution、getBillingTrend）都增加了startDate和endDate参数
- 更新页面组件以支持自定义日期范围选择
- 增强数据查询逻辑以支持灵活的时间范围过滤
- 添加DatePickerWithRange组件用于自定义日期范围选择
...
- **新增**：DatePickerWithRange组件提供自定义日期范围选择功能
...
**章节来源**
- [src/components/date-picker-with-range.tsx:1-92](file://src/components/date-picker-with-range.tsx#L1-L92)
- [src/app/(dashboard)/page.tsx](file://src/app/(dashboard)/page.tsx#L120-L129)
...
Page --> DatePicker["date-picker-with-range.tsx"]
Page --> Charts["各可视化组件"]
Charts --> ECharts["ECharts 库"]
Charts --> UI
DatePicker --> DayPicker["react-day-picker"]
```
...
- **新增**：日期范围问题
  - 现象：自定义日期范围查询无数据或数据不准确
  - 排查：确认 startDate 和 endDate 参数格式正确；检查时区处理；验证数据库时间戳格式
...
**章节来源**
- [src/server/api/routers/dashboard.ts:11-605](file://src/server/api/routers/dashboard.ts#L11-L605)
