...
- [src/server/api/routers/dashboard.ts:9-454](file://src/server/api/routers/dashboard.ts#L9-L454)
...
**更新** 最近 IP 请求组件新增了智能分页功能，包括省略号页码生成算法，支持超过 7 页时的智能显示策略。
...
**图表来源**
- [src/app/(dashboard)/components/recent-ip-requests.tsx](file://src/app/(dashboard)/components/recent-ip-requests.tsx#L45-L78)
- [src/components/ui/pagination.tsx:1-130](file://src/components/ui/pagination.tsx#L1-L130)
...
- 活动项
  ...
  - 时间格式化：相对时间
  - 提供商颜色映射：区分 OpenAI、Anthropic、Google、DeepSeek、Kimi、MiniMax 等
  - 详情展示：提供商标签、Token 数字化显示

**更新** 活动项组件新增了对 Kimi 和 MiniMax 提供商的支持，扩展了颜色映射方案。
...
- [src/app/(dashboard)/components/recent-ip-requests.tsx](file://src/app/(dashboard)/components/recent-ip-requests.tsx#L51-L78)
...
- [src/app/(dashboard)/components/recent-ip-requests.tsx](file://src/app/(dashboard)/components/recent-ip-requests.tsx#L35-L38)
...
**章节来源**
- [src/server/api/routers/dashboard.ts:11-454](file://src/server/api/routers/dashboard.ts#L11-L454)
...
- [src/app/(dashboard)/components/recent-ip-requests.tsx](file://src/app/(dashboard)/components/recent-ip-requests.tsx#L80-L92)
- [src/lib/date.ts:3-10](file://src/lib/date.ts#L3-L10)

### 提供商颜色映射
- 模型分布与最近 IP 请求均使用颜色映射
- 支持 OpenAI、Anthropic、Google、DeepSeek、Kimi、MiniMax 等
- 默认灰色，深浅色模式下分别对应不同透明度与对比度

**更新** 提供商颜色映射现已扩展至 Kimi 和 MiniMax，提供更丰富的颜色方案支持。
...
### 智能分页算法
- 当总页数 ≤ 7 时：显示所有页码
- 当总页数 > 7 时：根据当前页位置智能显示省略号
  - 当前页在前 3 页：显示 1, 2, 3, 4, 'ellipsis', totalPages
  - 当前页在末尾 3 页：显示 1, 'ellipsis', totalPages-3, totalPages-2, totalPages-1, totalPages
  - 否则：显示 1, 'ellipsis', currentPage-1, currentPage, currentPage+1, 'ellipsis', totalPages
