...
- **更新**：区域热力图组件内部重构，工具函数已移动到独立的utils.ts文件，支持世界地图视图的地区数据规范化
- **更新**：新增 normalizeToWorldMapRegion 函数和 PROVINCE_TO_COUNTRY_MAP 映射，支持世界地图视图的地区数据规范化
- **更新**：新增 getLocalizedCountryName 函数，实现 tooltip 和 legend 文本的国际化处理
...
- 功能要点
  ...
  - **更新**：新增世界地图视图支持，通过 normalizeToWorldMapRegion 函数进行地区数据规范化
  - **更新**：tooltip 和 legend 文本支持国际化，通过 getLocalizedCountryName 函数实现
- 数据处理
  - 将后端返回的地区分布数据映射为 ECharts 所需格式
  - 计算最大值用于视觉映射范围
  - 支持中国地图和世界地图两种视图
  - **更新**：地图工具函数分离到 utils.ts 文件，包括省份映射、国家映射和数据规范化
  - **更新**：新增 PROVINCE_TO_COUNTRY_MAP 映射，支持美国各州、日本都道府县等地区的国家级别归类
- 地图切换
...
GetData --> NormalizeData["数据规范化<br/>normalizeToWorldMapRegion"]
NormalizeData --> CalcMax["计算最大值<br/>用于视觉映射"]
...
- [src/app/(dashboard)/components/region-headmap-chart/utils.ts](file://src/app/(dashboard)/components/region-headmap-chart/utils.ts#L289-L304)
...
- 功能要点
  ...
  - 国家名称映射：中英文国家名称对照表
  - **新增**：PROVINCE_TO_COUNTRY_MAP 映射：支持美国各州、日本都道府县、英国各地区等地区的国家级别归类
  - **新增**：normalizeToWorldMapRegion 函数：将地区名称转换为国家级别名称，用于世界地图显示
  - **新增**：getLocalizedCountryName 函数：根据语言环境返回本地化国家名称
  - 数据接口定义：RegionDistributionItem 接口定义
- 模块化改进
  - 提高代码复用性和可维护性
  - 减少组件文件体积，专注于业务逻辑
  - 便于单元测试和功能扩展
  - **新增**：支持世界地图视图的完整数据处理流程
...
- **新增**：仪表板组件的国际化支持，包括地图视图切换、tooltip 和 legend 文本

```mermaid
flowchart TD
I18nSystem["国际化系统"] --> Messages["消息文件<br/>en.json/zh.json"]
I18nSystem --> Components["组件国际化<br/>Reports/Pagination/SidebarNav/Dashboard"]
Components --> Reports["报表页面<br/>多语言界面"]
Components --> Pagination["分页组件<br/>导航文本"]
Components --> SidebarNav["导航菜单<br/>菜单项文本"]
Components --> Dashboard["仪表板组件<br/>地图视图/tooltip/legend"]
...
- **更新**：区域热力图性能优化
  ...
  - 地图数据缓存和重复注册避免
  - 动态加载优化，按需获取地图数据
  - **新增**：normalizeToWorldMapRegion 函数的高效映射算法
  - **新增**：getLocalizedCountryName 函数的本地化缓存机制
- **新增**：通用工具函数性能优化
...
- **更新**：区域热力图工具函数问题
  ...
  - **新增**：检查 normalizeToWorldMapRegion 函数的映射逻辑
  - **新增**：验证 PROVINCE_TO_COUNTRY_MAP 映射表的完整性
  - **新增**：确认 getLocalizedCountryName 函数的语言环境处理
- **新增**：世界地图视图显示异常
  - 现象：世界地图显示地区名称而非国家名称
  - 排查：确认 normalizeToWorldMapRegion 函数正确执行
  - 检查 PROVINCE_TO_COUNTRY_MAP 映射表中是否存在对应的地区映射
- **新增**：tooltip 和 legend 文本国际化失效
  - 现象：tooltip 或 legend 显示英文而非本地化文本
  - 排查：确认 getLocalizedCountryName 函数正确返回本地化名称
  - 检查 locale 状态和消息文件的加载情况
- **新增**：通用工具函数 cn 函数问题
  - 现象：CSS 类名合并异常或样式冲突
...
- 地区热力图
  ...
  - **更新**：支持世界地图视图，通过 normalizeToWorldMapRegion 函数进行数据规范化
  - **更新**：tooltip 和 legend 文本支持国际化
- 最近 IP 请求
  - 配置项：数据数组、加载态
  - 交互：分页导航、相对时间格式化
- 最近活动
...
- 地区分布
  ...
  - **新增**：支持自定义日期范围的地区分布数据
  - **更新**：新增 normalizeToWorldMapRegion 函数处理世界地图视图的数据规范化
- 模型分布
...
- 测试友好：便于单元测试和功能扩展
- **新增**：支持世界地图视图的完整数据处理流程
- **新增**：PROVINCE_TO_COUNTRY_MAP 映射表，支持美国各州、日本都道府县等地区的国家级别归类
- **新增**：normalizeToWorldMapRegion 函数，将地区名称转换为国家级别名称
- **新增**：getLocalizedCountryName 函数，实现 tooltip 和 legend 文本的国际化处理
