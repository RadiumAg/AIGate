...
## 更新摘要
**变更内容**
- 新增 CombinedTrendChart 复合趋势图组件，整合请求量、令牌消耗和成本分析
- 增强区域热力图组件的地图切换功能，支持中国地图和世界地图切换
- 更新仪表板页面布局，将复合趋势图集成到现有架构中
- 扩展后端 getUsageTrend 接口以支持复合数据格式
...
**更新** 新增 CombinedTrendChart 复合趋势图组件，提供统一的请求量、令牌消耗和成本分析视图，增强区域热力图的地图切换功能，支持中国地图和世界地图的无缝切换。
...
- [src/server/api/routers/dashboard.ts:1-577](file://src/server/api/routers/dashboard.ts#L1-L577)
...
- [src/messages/zh.json:47-64](file://src/messages/zh.json#L47-L64)
- [src/messages/en.json:47-64](file://src/messages/en.json#L47-L64)
...
**更新** 新增 CombinedTrendChart 复合趋势图组件，支持请求量、令牌消耗和成本的统一展示；增强区域热力图组件的地图切换功能。
...
**更新** getUsageTrend 接口现在返回包含 requests、tokens 和 cost 字段的复合数据格式，支持 CombinedTrendChart 组件的展示需求。

```mermaid
flowchart TD
QStart["输入: startDate,endDate,days"] --> Range["推导查询范围"]
Range --> Query["Drizzle 查询 usageRecords 表"]
Query --> Group["按日期分组聚合<br/>requests/tokens/cost"]
...
- [src/server/api/routers/dashboard.ts:334-404](file://src/server/api/routers/dashboard.ts#L334-L404)
- [src/server/api/routers/dashboard.ts:406-458](file://src/server/api/routers/dashboard.ts#L406-L458)
...
**更新** 新增 Dashboard.cost、Dashboard.dataPeriod、Dashboard.chinaMap、Dashboard.worldMap 等翻译键值，支持复合趋势图和地图切换功能。
...
**更新** 新增 CombinedTrendChart 和增强的 RegionHeatmapChart 组件，扩展了图表组件生态。
...
## 结论
AIGate 监控与分析系统通过清晰的前后端职责划分与类型安全的 tRPC 接口，实现了仪表板的高效数据展示。新增的 CombinedTrendChart 复合趋势图组件提供了统一的请求量、令牌消耗和成本分析视图，增强了区域热力图的地图切换功能，支持中国地图和世界地图的无缝切换。ECharts 图表组件覆盖趋势、分布与地理可视化，配合 Drizzle ORM 的聚合查询与 Winston 日志体系，满足了实时监控、性能优化与运维可观测性的需求。后续可在高频字段建立索引、引入缓存层与更细粒度的指标埋点，进一步提升系统性能与可维护性。

**更新** 新增复合趋势图和地图切换功能显著提升了监控分析的可视化能力和用户体验。
