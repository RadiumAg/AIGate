...
## 更新摘要
**所做更改**
- 更新了 Vercel 配置简化部分，反映从复杂构建系统简化为单一重写规则的变更
- 更新了 CI/CD 工作流优化部分，反映移除 --prod 参数和增加 DATABASE_URL、REDIS_URL 环境变量支持
- 新增了 Vercel 部署架构图，展示简化后的部署流程
- 更新了环境变量管理部分，强调演示模式和生产环境变量的区别
...
- [Dockerfile.migrate:1-15](file://Dockerfile.migrate#L1-L15)
...
- [Dockerfile:1-52](file://Dockerfile#L1-L52)
- [docker-compose.yml:1-84](file://docker-compose.yml#L1-L84)
- [deploy.sh:1-168](file://deploy.sh#L1-L168)
...
### Vercel 配置简化与 CI/CD 工作流优化

**更新** 反映了 Vercel 配置从复杂构建系统简化为单一重写规则，以及 CI/CD 工作流的优化

#### Vercel 部署配置简化
AIGate 的 Vercel 配置已简化为单一重写规则，将所有路由重定向到 `/index.html`，这使得 Next.js 的客户端路由能够正常工作：

```mermaid
flowchart TD
Start["Vercel 请求"] --> Check["检查路由匹配"]
Check --> Match{"匹配 /(.*) ?"}
Match --> |是| Rewrite["重写到 /index.html"]
Match --> |否| Continue["继续其他处理"]
Rewrite --> SPA["单页应用路由处理"]
Continue --> End["结束"]
SPA --> End
```
...
#### CI/CD 工作流优化
GitHub Actions 工作流已优化，移除了 `--prod` 参数，增加了对数据库和 Redis 环境变量的支持：
...
GH->>VC : 部署命令移除 --prod 参数
VC->>ENV : 读取 DATABASE_URL
VC->>ENV : 读取 REDIS_URL
VC->>ENV : 读取 NEXT_PUBLIC_DEMO_MODE
VC->>VC : 执行部署
VC-->>GH : 返回部署 URL
```

**图表来源**
- [.github/workflows/vercel-deploy.yml:58-66](file://.github/workflows/vercel-deploy.yml#L58-L66)
...
**图表来源**
- [docker-compose.yml:62-75](file://docker-compose.yml#L62-L75)
- [Dockerfile.migrate:1-15](file://Dockerfile.migrate#L1-L15)
...
**图表来源**
- [src/lib/schema.ts:28-95](file://src/lib/schema.ts#L28-L95)
...
**更新** 强调了演示模式和生产环境变量的区别，以及 Vercel 部署时的环境变量管理
...
- [deploy.sh:135-145](file://deploy.sh#L135-L145)
...
## 结论
AIGate 的部署架构以 Docker 多阶段构建为基础，结合 Docker Compose 实现应用、数据库与缓存的一体化编排。通过独立迁移容器与环境变量管理，实现可重复、可审计的生产级部署。最新的 Vercel 配置简化和 CI/CD 工作流优化进一步提升了部署效率和可靠性。建议在生产环境中引入负载均衡与反向代理、Redis 集群、集中式监控与备份策略，并制定完善的故障转移与灾难恢复预案。
...
**章节来源**
- [deploy.sh:8-16](file://deploy.sh#L8-L16)

### 环境变量清单
- 数据库连接串：DATABASE_URL  
- Redis 连接串：REDIS_URL  
- NextAuth 秘钥与回调：NEXTAUTH_SECRET、NEXTAUTH_URL  
- 演示模式控制：NEXT_PUBLIC_DEMO_MODE、DEMO_MODE  
