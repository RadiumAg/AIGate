...
## 更新摘要
**所做更改**
- 更新字体栈优化章节，反映从 Geist 字体简化为 'Sora', 'Noto Sans SC' 的优化
- 更新仪表板组件样式章节，反映移除 hover 动画效果的性能优化
- 更新 UI 组件样式章节，反映按钮、对话框、表格等组件的 hover 动画优化
- 更新架构总览图，展示优化后的样式系统和性能考量
...
- [src/components/dashboard-layout/sidebar-footer.tsx:1-172](file://src/components/dashboard-layout/sidebar-footer.tsx#L1-L172)
...
**更新** UI 组件系统已进行性能优化，移除了不必要的 hover 动画效果，提升了交互性能和视觉稳定性。
...
- 定制策略
  ...
  - **性能优化**：移除复杂的 hover 动画过渡，保留必要的视觉反馈，提升交互流畅度。
- 示例组件
  - 按钮：提供多种变体与尺寸，适配不同交互层级。**优化后**移除了复杂的 scale 动画，保留基础的背景色变化。
  - 对话框：内置玻璃态背景、模糊与投影，提升信息密度与层次感。**优化后**简化了动画效果，提升打开关闭的响应速度。
  - 表格：在容器层面叠加玻璃态与阴影，增强可读性与对比度。**优化后**移除了行悬停时的复杂动画，仅保留基础的背景色变化。
...
- 全局样式
  ...
  - **字体栈优化**：使用 'Sora', 'Noto Sans SC' 作为主要字体，简化字体回退机制，提升加载性能。
- 主题系统
...
### 简化字体栈与性能优化
- 字体选择
  - **更新** 采用 'Sora', 'Noto Sans SC' 作为主要字体，简化了字体回退机制，提升加载性能。
  - 通过 CSS 变量 `--font-sans` 实现字体栈的统一管理。
- 国际化支持
  - Sora 字体提供良好的拉丁字符支持，Noto Sans SC 支持中文字符集。
  - 在 `globals.css` 中通过 `font-family` 属性统一应用字体栈。
- 性能优化
  - 简化字体回退列表，减少字体加载失败时的降级处理复杂度。
  - 通过 CSS 变量实现字体样式的集中管理，减少重复定义。
  - 移除复杂的字体加载检测逻辑，提升首屏渲染性能。

```mermaid
flowchart TD
FontInit["字体初始化"] --> DefineVars["定义 CSS 变量<br/>--font-sans: 'Sora', 'Noto Sans SC'"]
DefineVars --> ApplyFonts["应用字体栈<br/>font-family: var(--font-sans)"]
ApplyFonts --> IntlSupport["国际化支持<br/>Sora + Noto Sans SC"]
IntlSupport --> PerfOpt["性能优化<br/>简化回退机制"]
PerfOpt --> Fallback["降级显示<br/>sans-serif"]
```

**图表来源**
- [src/app/globals.css:131-136](file://src/app/globals.css#L131-L136)

### 字体栈配置与回退机制
- 主字体栈
  - `--font-sans`: `'Sora', 'Noto Sans SC', sans-serif` 作为主要无衬线字体。
- 系统回退
  - 通过 `font-family` 属性提供简化的系统字体回退列表。
  - 确保在不同操作系统和浏览器中的兼容性。
- 组件集成
...
**更新** 性能优化已扩展到样式系统，移除了不必要的 hover 动画效果。
...
- 样式与主题
  ...
  - **字体栈优化**：采用 'Sora', 'Noto Sans SC' 简化字体回退机制，提升加载性能。
  - **性能优化**：移除按钮、对话框、表格等组件的复杂 hover 动画，保留基础的视觉反馈，提升交互流畅度。
- 开发体验
...
- **字体栈优化**：采用 'Sora', 'Noto Sans SC' 简化字体回退机制，提升加载性能
- **UI 组件性能优化**：移除不必要的 hover 动画效果，提升交互流畅度和视觉稳定性
...
- 最佳实践
  ...
  - **性能优化**：在 UI 组件中避免使用复杂的 hover 动画效果，保留基础的视觉反馈即可。