...
- [.agents/skills/liquid-glass/examples.md](file://.agents/skills/liquid-glass/examples.md)
- [.calude/skills/liquid-glass/SKILL.md](file://.calude/skills/liquid-glass/SKILL.md)
- [.calude/skills/liquid-glass/examples.md](file://.calude/skills/liquid-glass/examples.md)
...
## 更新摘要
**变更内容**
- 新增 Apple Liquid Glass UI 组件系统章节，详细介绍液体玻璃视觉效果和多组件变体
- 更新 UI 规范文档，包含 Liquid Glass 样式指南和实现细节
- 新增 Liquid Glass 组件实现示例，涵盖 Glass Card、Glass Button、Glass Navbar 等组件
- 更新演示模式开发章节，包含 Liquid Glass 组件的演示数据支持
- 新增 Liquid Glass 性能优化和无障碍访问指南
...
5. [详细组件分析](#详细组件分析)
6. [开发工具集成](#开发工具集成)
7. [Apple Liquid Glass UI 组件系统](#apple-liquid-glass-ui-组件系统)
8. [演示模式开发](#演示模式开发)
9. [依赖分析](#依赖分析)
10. [性能考虑](#性能考虑)
11. [故障排查指南](#故障排查指南)
...
## 简介
本指南面向 AIGate 项目的开发者，提供从环境搭建、代码规范、TypeScript 配置、调试与测试、到贡献与发布流程的全流程说明。AIGate 是一个基于 Next.js 16 + tRPC + Redis 的智能 AI 网关管理系统，支持配额控制、多模型代理与现代化 UI。**最新更新**：项目集成了 React Scan、React Grab 和 MCP 开发工具，显著提升组件渲染性能可视化和开发效率。**新增功能**：Apple Liquid Glass UI 组件系统，提供液体玻璃视觉效果和多组件变体，包含 Glass Card、Glass Button、Glass Navbar 等组件实现。
...
- **新增**：.agents/skills/liquid-glass 和 .calude/skills/liquid-glass 提供 Apple Liquid Glass UI 组件系统文档
...
A --> F[".agents/skills/liquid-glass 液体玻璃组件系统"]
A --> G[".calude/skills/liquid-glass 液体玻璃组件系统"]
A --> H["配置文件<br/>tsconfig.json / eslint.config.mjs / .prettierrc / next.config.ts / tailwind.config.js / postcss.config.mjs / drizzle.config.ts / components.json"]
B --> B1["app 页面与路由"]
...
B --> B5["types 类型定义"]
B --> B6["utils 工具"]
B2 --> B21["UI 组件库<br/>shadcn/ui + Liquid Glass"]
...
E --> E2["CLI 工具参考"]
E --> E3["组件管理规则"]
F --> F1["Liquid Glass 技能说明"]
F --> F2["组件实现示例"]
G --> G1["Liquid Glass 技能说明"]
G --> G2["组件实现示例"]
```
...
- **新增**：Apple Liquid Glass 组件系统，提供液体玻璃视觉效果
...
## 架构总览
AIGate 采用前后端一体化的 Next.js App Router 架构，服务端通过 tRPC 暴露类型安全 API，前端使用 React 组件与 shadcn/ui 构建管理后台，数据持久化由 PostgreSQL + Redis 提供，日志系统采用 Winston。**最新架构**：集成了现代化开发工具链，包括性能监控、组件管理与 AI 辅助开发，**新增演示模式支持**，为测试和演示提供独立的数据环境，**新增 Apple Liquid Glass UI 组件系统**，提供现代化的液体玻璃视觉效果。
...
END
subgraph "液体玻璃组件层"
GLASSCOMP["Glass Card<br/>Glass Button<br/>Glass Navbar"]
GLASSUTIL["Glass Filter<br/>Glass Utility"]
GLASSANIM["动画与交互<br/>过渡效果"]
END
FE --> UI
FE --> GLASS
FE --> TRPC
...
- **新增**：Apple Liquid Glass 样式系统
  - CSS 自定义属性定义：--glass-bg、--glass-blur、--glass-saturate 等
  - 液体玻璃视觉效果：backdrop-blur、backdrop-saturate、边框渐变
  - 深色模式适配：深色背景下的玻璃效果优化
- PostCSS
...
## Apple Liquid Glass UI 组件系统

### 液体玻璃视觉效果概述
Apple Liquid Glass（iOS 26 / macOS Tahoe）风格的 UI 组件系统，提供现代化的半透明视觉效果。该系统包含四个视觉层次，通过 CSS 自定义属性和 Tailwind CSS 类实现一致的液体玻璃效果。

### 核心视觉特征
液体玻璃效果包含四个不同的视觉层次，从底部到顶部堆叠：

1. **模糊层** - `backdrop-filter: blur()` 模糊玻璃背景内容
2. **色调层** - 半透明背景色为玻璃着色
3. **光泽层** - 内部高光和边缘发光模拟光线折射
4. **内容层** - 实际的 UI 内容放置在顶部

额外视觉特征：
- 边缘有微妙的内部发光（inset box-shadow 与白色/半透明）
- 背景色渗透并增强饱和度（saturate(150-180%)）
- 微弱的外部阴影提供深度分离
- 圆角边框（border-radius: 1rem 到 2rem）
- 可选的 SVG feDisplacementMap 过滤器用于折射畸变
...
| 变体 | 背景 | 模糊 | 高光强度 | 使用场景 |
|------|------|------|----------|----------|
| `default` | `white/15` | `20px` | 中等 | 卡片、面板 |
| `thin` | `white/8` | `12px` | 低 | 覆盖层、提示框 |
| `thick` | `white/25` | `28px` | 高 | 模态框、导航栏 |
| `colored` | `{color}/15` | `20px` | 中等 | 强调元素 |

### 深色模式支持
始终提供深色模式变体：

- 减少背景不透明度：`white/15` -> `black/20`
- 减少高光强度：`rgba(255,255,255,0.6)` -> `rgba(255,255,255,0.15)`
- 减少边框不透明度：`white/25` -> `white/8`
- 保持模糊和饱和度值相似

### 动画指南
液体玻璃交互应该感觉有机：

- **悬停**：轻微放大（`scale(1.02)`）并增加高光
- **按下**：缩小（`scale(0.98)`）并减少高光
- **过渡**：使用弹簧般的三次贝塞尔曲线：`cubic-bezier(0.34, 1.56, 0.64, 1)`
- **持续时间**：悬停 200-300ms，按下 150ms
...
### 无障碍访问
- **对比度**：如果文本对比度比率 < 4.5:1，添加更深的色调层或使用 `text-shadow`
- **运动减少**：将动画包装在 `motion-safe:` Tailwind 前缀中
- **回退**：对于不支持 `backdrop-filter` 的浏览器，提供纯半透明背景
...
### 性能优化
- `backdrop-filter` 触发 GPU 合成；避免在许多重叠元素上使用
- 列表和重复项目优先使用单层方法
- 将多层方法保留给英雄元素和模态框
- 避免直接动画 `backdrop-filter` 或 `filter` 值

### 与其他 UI 组件的集成
液体玻璃组件可以与现有的 shadcn/ui 组件无缝集成，通过覆盖样式类实现统一的视觉效果。推荐在对话框、导航栏、卡片和输入框等组件中使用液体玻璃效果。

**章节来源**
- [.agents/skills/liquid-glass/SKILL.md:1-157](file://.agents/skills/liquid-glass/SKILL.md#L1-L157)
- [.agents/skills/liquid-glass/examples.md:1-346](file://.agents/skills/liquid-glass/examples.md#L1-L346)
- [.calude/skills/liquid-glass/SKILL.md:1-157](file://.calude/skills/liquid-glass/SKILL.md#L1-L157)
- [.calude/skills/liquid-glass/examples.md:1-346](file://.calude/skills/liquid-glass/examples.md#L1-L346)
- [readme/ui-rule.md:34-41](file://readme/ui-rule.md#L34-L41)
- [src/app/globals.css:1-138](file://src/app/globals.css#L1-L138)
...
P --> GLASS["液体玻璃组件: Glass Card / Button / Navbar"]
...
- 构建与运行
  ...
  - **液体玻璃性能**：backdrop-filter 触发 GPU 合成，注意使用场景
- 数据访问
...
- UI 与网络
  ...
  - tRPC 类型安全减少序列化开销
  - **液体玻璃优化**：避免在大量重叠元素上使用 backdrop-filter
- 日志与监控
...
- 本地开发常见问题
  ...
  - **液体玻璃问题**：检查 backdrop-filter 支持和 CSS 变量定义
- Lint 与格式化
...
- **液体玻璃故障排查**
  - 液体玻璃效果不显示：检查浏览器对 backdrop-filter 的支持
  - CSS 变量未生效：确认 CSS 自定义属性定义正确
  - 组件样式冲突：检查 Tailwind CSS 类的优先级
...
## 结论
本指南提供了 AIGate 从环境搭建到日常开发与维护的完整路径。**最新更新**：通过集成 React Scan、React Grab 和 MCP 开发工具，项目现在具备了完整的现代化开发工具链。**新增功能**：Apple Liquid Glass UI 组件系统，提供现代化的液体玻璃视觉效果，包含 Glass Card、Glass Button、Glass Navbar 等组件实现。**新增功能**：演示模式开发脚本（dev:demo 和 build:demo）简化了演示环境的测试流程，为开发者提供了独立的测试和演示环境。遵循本文的配置与流程，可确保团队在 TypeScript、ESLint/Prettier、Tailwind/shadcn/ui、tRPC、Drizzle/PostgreSQL、Redis 与 Next.js 16 生态下高效协作，同时享受先进的性能监控、AI 辅助开发、液体玻璃视觉效果和演示模式测试体验。
...
- **新增**：演示模式配置
  ...
  - 无需额外配置即可使用演示模式功能
- **新增**：液体玻璃组件集成
  - 在根布局中添加 GlassFilter 组件提供折射畸变支持
  - 使用 GlassCard、GlassButton 等组件实现液体玻璃效果
  - 确保 CSS 变量正确配置以支持深色模式
- 启动开发服务器
  - pnpm dev（常规开发）
...
- 提交建议
  ...
  - **新增**：开发工具使用规范
    ...
    - **液体玻璃组件规范**：遵循 Apple Liquid Glass 设计原则，合理使用 backdrop-filter
...
- **新增**：开发工具调试
  ...
  - **液体玻璃调试**：检查 backdrop-filter 支持，验证 CSS 变量效果
...
- **新增**：演示模式测试
  ...
  - 构建测试：验证演示模式的生产构建过程
- **新增**：液体玻璃组件测试
  - 视觉回归测试：验证液体玻璃效果在不同浏览器中的表现
  - 深色模式测试：确保深色模式下的对比度和可读性
  - 性能测试：检查 backdrop-filter 对性能的影响
  - 无障碍测试：验证对比度、运动减少和键盘导航支持
...
- **新增**：开发工具集成标准
  ...
  - **液体玻璃组件标准**：遵循 Apple Liquid Glass 设计原则，合理使用 CSS 自定义属性
  - **性能优化标准**：避免在大量元素上使用 backdrop-filter，注意 GPU 合成开销
...
- **新增**：开发工具贡献
  ...
  - **液体玻璃组件贡献**：组件需遵循 Apple Liquid Glass 设计规范，提供深色模式支持
  - **性能优化贡献**：液体玻璃组件需考虑性能影响，提供使用指导和最佳实践
...
- **新增**：液体玻璃组件发布
  - **组件文档发布**：更新 Liquid Glass 组件的使用文档和示例
  - **样式系统发布**：确保 CSS 变量和自定义属性在生产环境中正常工作
  - **性能监控**：监控液体玻璃效果对页面性能的影响
  - **兼容性测试**：验证不同浏览器对 backdrop-filter 的支持情况
