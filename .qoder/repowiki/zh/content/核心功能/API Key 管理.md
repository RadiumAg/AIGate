...
- 前端定价配置界面：新增可折叠的定价配置面板，使用 Collapsible、CollapsibleContent、CollapsibleTrigger 组件提供更好的用户体验
- 后端数据转换：增强 API Key 路由处理，支持定价数据的转换与存储
- 翻译资源完善：新增定价配置相关的多语言翻译键值
- 数据库模式扩展：API Keys 表新增定价相关字段定义
- UI 组件优化：引入 Radix UI Collapsible 组件实现可折叠界面
...
**更新** 系统已完成定价配置系统的集成，支持为每个 API Key 设置自定义的输入和输出 Token 定价。新增的定价配置功能允许管理员根据不同提供商或特定需求设置差异化的成本计算标准，为后续的成本分析和配额管理提供更精确的数据支持。定价配置界面采用可折叠设计，使用 Radix UI Collapsible 组件提供更好的用户体验。
...
CollapsibleUI["可折叠UI组件<br/>Collapsible/CollapsibleTrigger"]
end
subgraph "国际化系统"
...
RadixCollapsible["Radix UI Collapsible<br/>可折叠组件"]
end
subgraph "后端"
...
- **前端页面**：提供密钥列表、新增/编辑、删除确认与状态切换的可视化操作，集成液体玻璃样式确认对话框系统和 Toast 通知系统，支持多语言界面。**更新**：新增定价配置面板，支持可折叠的定价设置界面，使用 Radix UI Collapsible 组件。
...
- **Radix UI 可折叠组件**：**新增** 基于 Radix UI 的 Collapsible 组件，提供可折叠的定价配置界面，支持动画过渡和状态管理。

**更新** 新增定价配置系统作为核心组件之一，提供灵活的 API Key 定价管理功能。系统支持自定义定价与系统默认定价的优先级处理，通过缓存机制提升性能。新增的 Radix UI 可折叠组件为定价配置界面提供了现代化的用户体验。
...
participant Collapsible as "可折叠组件"
participant API as "apiKeyRouter"
participant DB as "数据库"
participant Cache as "Redis"
Admin->>I18n : 切换语言
I18n->>UI : 更新翻译资源
UI->>Collapsible : 展开定价配置面板
Collapsible->>Pricing : 配置 API Key 定价
...
API-->>UI : 返回删除结果
UI->>Toast : 显示成功/错误通知
Note over I18n,Pricing : 国际化系统 + 定价配置系统 + 液体玻璃样式确认对话框 + 基于 Sonner 的 Toast 通知 + Radix UI 可折叠组件
```
...
- **AddApiKeyDialog**：表单校验（名称、API Key 必填），动态占位符与提供商提示，支持新增与编辑。**更新**：新增定价配置面板，支持可折叠的定价设置界面，包含输入验证和占位符提示，使用 Radix UI Collapsible 组件实现。
...
**更新** 新增定价配置面板到 AddApiKeyDialog 组件，提供完整的定价设置功能。定价面板采用 Radix UI Collapsible 组件实现的可折叠设计，包含输入验证、占位符提示和多语言支持。新增的可折叠组件提供了更好的用户体验和界面组织。
...
Page->>Table : 渲染表格
Admin->>Dialog : 新增/编辑
Dialog->>Collapsible : 展开定价配置面板
Collapsible->>Pricing : 配置定价设置
...
### Radix UI 可折叠组件系统
**新增** 系统已集成 Radix UI 的 Collapsible 组件，为定价配置界面提供可折叠功能。

- **组件结构**：
  - `Collapsible`：根组件，管理折叠状态
  - `CollapsibleTrigger`：触发器，点击展开/收起
  - `CollapsibleContent`：内容区域，显示折叠内容
- **动画效果**：支持平滑的展开/收起动画，使用 CSS transition 属性
- **状态管理**：通过 `data-state` 属性管理展开/收起状态
- **图标联动**：ChevronDown 图标随状态旋转 180 度
- **样式定制**：支持自定义类名和样式属性
- **无障碍支持**：符合 WAI-ARIA 标准，支持键盘导航
...
- **界面设计**：
  - 可折叠面板：通过 ChevronDown 图标控制展开/收起，使用 Radix UI Collapsible 组件
...
- **多语言支持**：
  - 定价配置标题："Pricing Configuration (Optional)" / "定价配置（可选）"
  - 描述文本："Set custom pricing for this API Key for cost calculation. Leave empty to use system default pricing"
  - 字段标签："Input Token Price" / "输入 Token 价格"
  - 单位说明："USD per million tokens" / "美元/百万 tokens"
- **动画效果**：使用 CSS transition 属性实现平滑的展开/收起动画
...
- **UI 组件集成**：**新增** Radix UI 可折叠组件与液体玻璃样式对话框组件的集成，提供现代化的用户界面。
...
Pricing --> DB
Pricing --> Cache["定价缓存"]
Collapsible["Radix UI 可折叠组件"] --> UI
Collapsible --> Pricing
LiquidGlass["液体玻璃对话框组件"] --> UI
```
...
- **可折叠组件性能**：**新增** Radix UI 可折叠组件使用 CSS 动画，性能开销最小化，支持硬件加速。

**更新** 新增定价配置系统的性能考虑，包括内存缓存机制、数据库查询优化和错误回退处理。新增的可折叠组件系统提供了高效的动画性能和状态管理。
...
- **定价配置问题**
  ...
  - **新增** 检查可折叠组件是否正常展开/收起，确认 ChevronDown 图标旋转效果。
- **可折叠组件问题**
  - **新增** 检查 Radix UI Collapsible 组件是否正确导入和使用。
  - 确认 CSS transition 动画是否正常工作。
  - 检查 data-state 属性是否正确更新。
  - 确认 ChevronDown 图标是否随状态正确旋转 180 度。
- **日志定位**
  - 查看日志模块输出的配额检查与 AI 请求记录，定位异常原因。

**更新** 新增定价配置相关的故障排除指南，包括字段格式验证、数据库类型检查、缓存机制测试等。新增可折叠组件相关的故障排除指南。
...
**更新** 新增的定价配置系统提供了灵活的 API Key 定价管理功能，支持自定义定价与系统默认定价的优先级处理。系统通过内存缓存机制提升性能，通过数据库模式扩展支持精确的定价存储。完整的多语言支持确保了全球化的用户体验，而增强的国际化系统为所有界面元素提供了统一的翻译资源。新增的 Radix UI 可折叠组件系统为定价配置界面提供了现代化的用户体验，通过 CSS 动画和状态管理实现了流畅的交互效果。
...
- **新增** 定价配置功能：在新增/编辑对话框中展开"定价配置"面板，使用可折叠组件实现，输入自定义的输入和输出 Token 价格，支持美元/百万 tokens 的格式。
- **新增** 可折叠组件使用：点击定价配置标题可展开/收起定价设置面板，ChevronDown 图标随状态旋转 180 度。
...
- **最佳实践**：
  ...
  - 定期检查和更新定价信息
  - 使用留空功能保持与系统默认定价的一致性
- **可折叠界面**：**新增** 使用 Radix UI Collapsible 组件实现，提供平滑的展开/收起动画和状态管理
