...
- 完善了定价配置的用户界面支持
- 集成了模型定价计算服务
- **新增** 演示模式统计功能增强，getDemoStats.getRecentIpRequests函数新增startDate和endDate参数支持，允许按日期范围过滤演示数据，与生产模式保持一致的日期过滤逻辑
...
3. **中国省份映ap**：支持完整的中国23个省份和地区
4. **国际地区映ap**：支持8个主要国际地区的英文名称映ap
...
- **更新** 区域映ap使用 Set 和 Map 数据结构优化查找性能
...
**新增** 定价功能和日期范围过滤功能的集成使得演示模式系统能够更好地模拟真实的成本计算场景和数据分析需求，为用户提供了更加完整和实用的演示体验。通过支持API Key级别的自定义定价、模型级别的默认定价和精确的日期范围查询，系统能够适应各种复杂的业务场景需求，为实际生产环境的部署提供了有力的技术支撑。