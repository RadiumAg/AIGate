...
## 更新摘要
**变更内容**
- 新增Toast通知系统，提供白名单规则管理的完整反馈机制
- 修复API Key选择值绑定问题，确保正确的值传递
- 集成全局通知组件，提供统一的用户体验
...
**更新** 系统现已集成Toast通知系统，提供创建、更新、删除和状态切换操作的成功和错误反馈，显著提升了用户体验和操作透明度。

## 项目结构
白名单控制涉及三层协作：
- 前端管理界面：规则表单与表格组件，支持规则创建、编辑、启用/禁用与删除，具备完整的Toast通知反馈。
...
- 前端表单与表格：提供规则配置、预设模板、快捷插入、键盘导航与状态切换，具备完整的Toast通知反馈。
- 通知系统：集成sonner通知组件，提供创建、更新、删除和状态切换操作的成功和错误反馈。
- 与配额控制协同：在AI聊天流接口中，先校验白名单规则再进行配额检查，确保访问控制与用量限制双保险。

**更新** 新增Toast通知系统，提供即时的操作反馈，包括成功提示和错误警告，显著提升了用户体验。

**章节来源**
- [src/lib/schema.ts:85-98](file://src/lib/schema.ts#L85-L98)
- [src/server/api/routers/whitelist.ts:22-221](file://src/server/api/routers/whitelist.ts#L22-L221)
- [src/lib/database.ts:421-545](file://src/lib/database.ts#L421-L545)
...
- [src/app/(dashboard)/users/page.tsx](file://src/app/(dashboard)/users/page.tsx#L35-L81)
- [src/components/ui/sonner.tsx:15-43](file://src/components/ui/sonner.tsx#L15-L43)
- [src/pages/api/ai/chat/stream.ts:32-49](file://src/pages/api/ai/chat/stream.ts#L32-L49)

## 架构总览
白名单控制贯穿"前端配置—后端校验—接口拦截—用量统计"的闭环。下图展示了从请求进入系统到完成白名单校验的关键路径，包括新增的通知反馈机制。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant UI as "用户界面<br/>users/page.tsx"
...
**图表来源**
- [src/lib/database.ts:421-449](file://src/lib/database.ts#L421-L449)

**章节来源**
- [src/lib/schema.ts:85-98](file://src/lib/schema.ts#L85-L98)
- [src/lib/database.ts:408-449](file://src/lib/database.ts#L408-L449)
...
**图表来源**
- [src/server/api/routers/whitelist.ts:22-221](file://src/server/api/routers/whitelist.ts#L22-L221)
- [src/lib/database.ts:293-579](file://src/lib/database.ts#L293-L579)

**章节来源**
- [src/server/api/routers/whitelist.ts:22-221](file://src/server/api/routers/whitelist.ts#L22-L221)
- [src/lib/database.ts:317-393](file://src/lib/database.ts#L317-L393)

### 前端规则表单与表格
- 表单特性
  - 策略选择、描述、优先级、关联API Key、用户ID生成规则、启用校验开关、校验规则。
  - 内置预设模板（如@ip、@email、@uuid、@any等），支持快捷插入与键盘导航。
  - **修复** API Key选择值绑定问题，确保正确的值传递和状态管理。
- 表格特性
...
- [src/app/(dashboard)/users/page.tsx](file://src/app/(dashboard)/users/page.tsx#L35-L81)
- [src/server/api/routers/whitelist.ts:67-148](file://src/server/api/routers/whitelist.ts#L67-L148)
- [src/lib/database.ts:354-393](file://src/lib/database.ts#L354-L393)
...
### Toast通知系统
- 通知组件集成
  - 在应用布局中集成Toaster组件，提供全局通知服务。
  - 支持多种通知类型：成功、错误、警告、信息、加载状态。
  - 自动主题适配，支持明暗模式切换。
- 操作反馈
  - **创建规则**：成功时显示"白名单规则创建成功"，失败时显示具体错误信息。
  - **更新规则**：成功时显示"白名单规则更新成功"，失败时显示具体错误信息。
  - **删除规则**：成功时显示"白名单规则删除成功"，失败时显示具体错误信息。
  - **状态切换**：成功时显示"白名单规则状态切换成功"，失败时显示具体错误信息。
- 用户体验
  - 即时反馈机制，提升操作透明度。
  - 统一的通知样式和交互，增强用户体验一致性。

**新增** 完整的Toast通知系统为白名单规则管理提供了即时的操作反馈，显著提升了系统的可用性和用户体验。

**章节来源**
- [src/app/(dashboard)/users/page.tsx](file://src/app/(dashboard)/users/page.tsx#L35-L81)
- [src/app/layout.tsx:53-53](file://src/app/layout.tsx#L53-L53)
- [src/components/ui/sonner.tsx:15-43](file://src/components/ui/sonner.tsx#L15-L43)
...
**章节来源**
- [src/lib/database.ts:408-419](file://src/lib/database.ts#L408-L419)
- [src/lib/schema.ts:85-98](file://src/lib/schema.ts#L85-L98)

### 动态规则更新与批量导入导出
- 动态更新
  - 通过管理界面的表单进行创建/更新/删除/切换状态，接口层提供统一入口，实时生效。
  - **新增** 完整的Toast通知反馈，提供即时的操作结果提示。
- 批量导入导出
...
**章节来源**
- [src/server/api/routers/whitelist.ts:67-192](file://src/server/api/routers/whitelist.ts#L67-L192)
- [src/app/(dashboard)/users/components/whitelist-rule-form.tsx](file://src/app/(dashboard)/users/components/whitelist-rule-form.tsx#L128-L282)
- [src/app/(dashboard)/users/page.tsx](file://src/app/(dashboard)/users/page.tsx#L35-L81)
...
- 组件耦合
  ...
  - **新增** 通知系统通过全局布局集成，为所有页面提供统一的通知服务。
- 外部依赖
...
```mermaid
graph LR
Form["whitelist-rule-form.tsx"] --> Page["users/page.tsx"]
Table["whitelist-rule-table.tsx"] --> Page
Page --> Router["whitelist.ts"]
Router --> DB["database.ts"]
DB --> Schema["schema.ts"]
API["pages/api/ai/chat/stream.ts"] --> DB
API --> Logger["logger-middleware.ts"]
Layout["layout.tsx"] --> Sonner["sonner.tsx"]
```
...
- 常见错误与原因
  ...
  - **新增** API Key冲突：更新时若目标API Key已被其他规则绑定则拒绝。
- 排查步骤
  - 检查规则状态与优先级是否正确。
  - 校验校验规则是否为有效正则表达式。
  - 确认API Key与规则的绑定关系是否冲突。
  - 查看日志中间件输出，定位具体失败环节。
  - **新增** 检查Toast通知是否正常显示，确认操作反馈是否正确。
- 相关参考
  - AI聊天流接口的错误处理与日志记录。

**章节来源**
- [src/pages/api/ai/chat/stream.ts:36-49](file://src/pages/api/ai/chat/stream.ts#L36-L49)
- [src/lib/logger-middleware.ts:32-67](file://src/lib/logger-middleware.ts#L32-L67)
- [src/server/api/routers/whitelist.ts:111-121](file://src/server/api/routers/whitelist.ts#L111-L121)

## 结论
白名单控制系统通过"规则优先级+正则校验+用户ID生成"的组合，实现了灵活而强大的访问控制。配合配额检查与日志记录，系统在保障安全的同时具备良好的可观测性与可维护性。

**更新** 新增的Toast通知系统显著提升了用户体验，提供了即时的操作反馈和错误提示。API Key选择值绑定问题的修复确保了系统的稳定性和可靠性。

建议在生产环境中结合缓存与索引策略进一步优化性能，并通过管理界面实现规则的动态治理与实时生效。Toast通知系统的集成使得系统管理更加直观和高效。
...
- **新增** 通知系统使用指南
  - Toast通知自动集成到应用布局中，无需额外配置即可使用。
  - 支持多种通知类型，可根据需要自定义样式和行为。

**章节来源**
- [README.md:52-82](file://README.md#L52-L82)
- [readme/project-description.md:57-103](file://readme/project-description.md#L57-L103)
- [src/app/layout.tsx:53-53](file://src/app/layout.tsx#L53-L53)
- [src/components/ui/sonner.tsx:15-43](file://src/components/ui/sonner.tsx#L15-L43)