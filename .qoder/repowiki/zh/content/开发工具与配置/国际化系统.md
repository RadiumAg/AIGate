...
**变更内容**
- 完全新版本的国际化系统实现
- 新增 I18nProvider 组件和 useTranslation 钩子
- 新增完整的多语言翻译资源（中文/英文）
- 新增动态语言切换功能和本地存储持久化
- 新增语言切换器组件和使用示例
- 更新所有现有组件以集成新的国际化系统
...
subgraph "使用示例"
J[src/app/(dashboard)/keys/components/api-key-table.tsx] --> E
K[src/app/login/page.tsx] --> E
L[src/app/(dashboard)/keys/components/add-api-key-dialog.tsx] --> E
M[src/app/(dashboard)/quotas/components/policy-form.tsx] --> E
N[src/app/(dashboard)/users/page.tsx] --> E
O[src/app/(dashboard)/quotas/components/policy-table.tsx] --> E
end
A -.-> J
A -.-> K
A -.-> L
...
系统采用分层的 JSON 结构组织翻译内容，包含完整的业务领域覆盖：
...
**图表来源**
- [src/messages/en.json:2-179](file://src/messages/en.json#L2-L179)
- [src/messages/zh.json:2-179](file://src/messages/zh.json#L2-L179)

**章节来源**
- [src/i18n/client.tsx:15-96](file://src/i18n/client.tsx#L15-L96)
- [src/messages/en.json:1-180](file://src/messages/en.json#L1-L180)
- [src/messages/zh.json:1-180](file://src/messages/zh.json#L1-L180)
...
class MessagesMap {
+Record~string, any~ zh
+Record~string, any~ en
}
I18nContextType --> Locale
I18nContextType --> MessagesMap
```

**图表来源**
- [src/i18n/client.tsx:8-19](file://src/i18n/client.tsx#L8-L19)
...
该组件展示了国际化在实际业务场景中的应用：

```mermaid
sequenceDiagram
participant C as ApiKeyTable组件
participant T as useTranslation钩子
participant P as I18nProvider
participant M as 翻译映射
C->>T : 调用 useTranslation()
T->>P : 获取翻译上下文
P->>M : 加载中文翻译
M-->>P : 返回翻译对象
P-->>T : 返回 t 函数
T-->>C : 返回翻译上下文
C->>T : t('ApiKey.name')
T->>M : 查找 Navigation.dashboard
M-->>T : 返回 "仪表板"
T-->>C : 返回 "仪表板"
```

**图表来源**
- [src/app/(dashboard)/keys/components/api-key-table.tsx:24](file://src/app/(dashboard)/keys/components/api-key-table.tsx#L24)
- [src/app/(dashboard)/keys/components/api-key-table.tsx:35](file://src/app/(dashboard)/keys/components/api-key-table.tsx#L35)
...
**章节来源**
- [src/app/(dashboard)/keys/components/api-key-table.tsx:1-183](file://src/app/(dashboard)/keys/components/api-key-table.tsx#L1-L183)
...
- [src/i18n/client.tsx:3-4](file://src/i18n/client.tsx#L3-L4)
...
- [src/i18n/client.tsx:5-6](file://src/i18n/client.tsx#L5-L6)
...
- [src/i18n/client.tsx:76-78](file://src/i18n/client.tsx#L76-L78)

## 结论

本国际化系统通过简洁而高效的架构设计，为 AIGate AI 网关管理系统提供了可靠的多语言支持。系统的主要优势包括：
