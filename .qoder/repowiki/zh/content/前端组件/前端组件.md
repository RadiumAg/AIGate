...
- KeysPage
  ...
  - 参考路径：[src/app/(dashboard)/keys/page.tsx](file://src/app/(dashboard)/keys/page.tsx#L1-L140)
...
- **配额管理页面 QuotasPage**
  ...
  - 参考路径：[src/app/(dashboard)/quotas/page.tsx](file://src/app/(dashboard)/quotas/page.tsx#L71-L75)

- **用户管理页面 UsersPage**
  - 在删除白名单规则时使用 confirm 函数进行确认。
  - 支持异步删除操作，删除成功后自动刷新数据。
  - 参考路径：[src/app/(dashboard)/users/page.tsx](file://src/app/(dashboard)/users/page.tsx#L94-L98)

- **API 密钥管理页面 KeysPage**
  - 在删除 API 密钥时使用 confirm 函数进行确认，支持自定义 onConfirm 回调。
  - 参考路径：[src/app/(dashboard)/keys/page.tsx](file://src/app/(dashboard)/keys/page.tsx#L65-L75)
