...
## 更新摘要
**所做更改**
- 新增 GitHub Actions CI/CD 工作流章节，详细介绍自动化部署流程
- 更新部署策略，包含本地部署与云端部署两种模式
- 新增 Vercel 集成配置说明
- 更新环境变量配置，支持 DEMO_MODE 和 NEXT_PUBLIC_DEMO_MODE
- 新增 CI/CD 流程图和部署架构图
...
**更新** 新增 GitHub Actions CI/CD 工作流，支持自动化部署、版本标签触发、Vercel 集成和 GitHub Release 创建功能，提供完整的持续集成与持续部署解决方案。
...
**更新** 端口配置语法更新为 ${APP_PORT:-3000} 格式，提供默认值支持；交互式配置功能简化环境变量设置流程；新增 CI/CD 自动化部署功能。
...
4. **构建应用**：执行 `pnpm run build:demo` 构建演示版本
...
- Vercel 部署时自动设置 `NEXT_PUBLIC_DEMO_MODE=true`

#### 构建脚本
- 使用 `pnpm run build:demo` 构建演示版本
- 支持演示模式下的特殊配置和功能
...
**更新** 新增的交互式配置功能显著提升了用户体验，简化了环境变量设置流程，配合简化的数据库迁移流程，使整体部署体验更加友好高效。同时，新增的 GitHub Actions CI/CD 工作流提供了完整的自动化部署解决方案，支持版本标签触发、Vercel 集成和 GitHub Release 创建，大大提高了开发效率和部署可靠性。
