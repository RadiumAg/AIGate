...
- 组件文件使用kebab-case：`user-profile.tsx`
- 工具文件使用kebab-case：`user-utils.ts`
- 类型文件使用kebab-case：`user-types.ts`

### 组件命名

- 组件名使用kebab-case：`user-profile`
- Props接口名：`user-profile-props`
- 类型别名：`user-status`
- 枚举名：`user-type`
...
```typescript
// 自定义Hook示例
const use-component-logic = (props: component-props) => {
...
```typescript
// 纯组件优化
export const pure-component = React.memo(({ value, onClick }: Props) => {
